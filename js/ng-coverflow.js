        ...
        .directive('ngCoverflow', [ '$timeout', function ($timeout) {
            return {
                ...
                link: function (scope, iElement, attrs) {
                    ...
                    var WRAP        = true;//attrs.wrapAround === 'true';
                    ...
                    iElement.on('mousewheel', function (event) {
						...
						if(event.deltaX == 0 && event.deltaY == 0 && (maxMove  < (movedIndex *moveThreshold))) {
							maxMove = moveThreshold+1;
						}
						if(maxMove  > (movedIndex *moveThreshold)){
							movedIndex = parseInt(maxMove / moveThreshold)+1;
                    ...
                    scope.$watch('selectedIndex', function (newValue, oldValue) {
                        // updateCurrentItem();
                        render();
                        var scrollTo = parseInt(scope.selectedIndex)-1;
                        var elt = jQuery(".new_comics .product_list md-list-item.pro_item:nth-child("+parseInt(scope.selectedIndex)+")");
                        var parentDiv = jQuery(".new_comics .product_list");
                        if(elt.length > 0){
// 							jQuery(".new_comics .product_list").animate({scrollTop: elt.offset().top}, "slow");
// 							parentDiv.scrollTop(elt.position().top);
							console.log(parentDiv.scrollTop()-parentDiv.innerHeight()+elt.offset().top);
							parentDiv.animate({ scrollTop: parentDiv.scrollTop()-parentDiv.innerHeight()-parentDiv.innerHeight()/2-parentDiv.height()+elt.offset().top },"slow");

						}
                    });

                    scope.$on('ng-coverflow:ng-repeat-done', function () {
                        scope.selectedIndex = 1;//Math.floor(iElement.find('li').length / 2);
                    });
